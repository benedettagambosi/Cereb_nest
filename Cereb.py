...
neuron_param = {
    ...
    'io': {'t_ref': 1.0, 'C_m': 189.0, 'tau_m': 11.0, 'V_th': -35.0, 'V_reset': -45.0, 'Vinit': -45.0, 'E_L': -45.0,
           'lambda_0': 1.2, 'tau_V': 0.8, 'I_e': -18.101, 'kadap': 1.928, 'k1': 0.191, 'k2': 0.091, 'A1': 1810.93,
           'A2': 1358.197,  # 'kadap': 1.5
...
# Connection weights
conn_weights = {'aa_goc': 1.2, 'aa_pc': 0.7, 'bc_pc': 0.3, 'dcnp_io': 3.0, 'gj_bc': 0.2, 'gj_sc': 0.2, 'glom_dcn': 0.05, 'pf_pc': 0.4, #0.007
                'pf_sc': 0.015, 'sc_pc': 0.3, \
                'glom_goc': 1.5, 'glom_grc': 0.15, 'goc_glom': 0.0, 'gj_goc': 0.3, 'goc_grc': 0.6, 'io_dcn': 0.1,
                'io_dcnp': 0.2,
                'io_bc': 1.0, 'io_sc': 1.0, 'io_pc': 300.0, 'pc_dcn': 0.4 * 1.2, 'pc_dcnp': 0.12 * 1.2, 'pf_bc': 0.015,
                'pf_goc': 0.05} #glom_dcn': 0.05,'io_dcn': 0.1, 'io_dcnp': 0.2,
# conn_weights = {'aa_goc': 1.2, 'aa_pc': 0.7, 'bc_pc': 0.3, 'dcnp_io': 3.0, 'gj_bc': 0.2, 'gj_sc': 0.2, 'glom_dcn': 0.05, \
#                 'glom_goc': 1.5, 'glom_grc': 0.15, 'goc_glom': 0.0, 'gj_goc': 0.3, 'goc_grc': 0.6, 'io_dcn': 0.1,
#                 'io_dcnp': 0.2,
#                 'io_bc': 1.0, 'io_sc': 1.0, 'io_pc': 350.0, 'pc_dcn': 0.4 * 1.2, 'pc_dcnp': 0.12 * 1.2, 'pf_bc': 0.015,
#                 'pf_goc': 0.05, 'pf_pc': 0.007,
#                 'pf_sc': 0.015, 'sc_pc': 0.3} #glom_dcn': 0.05,'io_dcn': 0.1, 'io_dcnp': 0.2,
# Connection delays
...
class Cereb_class:
    def __init__(self, nest, hdf5_file_name, cortex_type, n_spike_generators='n_glomeruli',
                 mode='external_dopa', experiment='active', dopa_depl=0, LTD=None, LTP=1e-3):
    ...
    def create_Cereb(self, nest_, pos_file, mode, experiment, dopa_depl, LTD, LTP):
        ...
        # with h5py.File(pos_file, 'r') as f:
        #     if plasticity:
        #         # create volume transmitters #TODO
        ...
        #         else:
        #             LTD1 = -0.09 #-1.0e-3*2
        #         LTP1 = LTP#0.0225 # from marco 1.0e-5
        #         nest_.SetDefaults('stdp_synapse_sinexp',
        #                           {"A_minus": LTD1,  # -1.0e-2
        #                            "A_plus": LTP1,  # 1.0e-3
        #                            "Wmin": 0.0,
        #                            "Wmax": 2.,#conn_weights['pf_pc'] * 10,  # 0.007
        #                            "vt": vt[0]})
        ...
        #         else:  # if plasticity
        #             if conn not in ["pf_pc", 'io_pc']:# TODO non è exponential?
        ...
        #                                "weight":  0.,#conn_weights[conn],
        #                                "delay": 1.})  # 1.0
        #                 # nest.Connect(np.array(pre, int)[pc_selected_ids], np.array(post, int)[pc_selected_ids],
        #                 #               {'rule': 'one_to_one'},  # connected one to one
        #                 #               {"model": "static_synapse",  # "receptor_type": conn_receptors[conn]
        #                 #                "receptor_type":3,
        #                 #                "weight":  2.,#conn_weights[conn],
        ...
        #                 # postsynaptic_cells = np.unique(post)
        ...
        #                 #         if len(indexes)==1:
        #                 #             pre_neurons = pre[indexes[0]]
        #                 #             post_neurons = post[indexes[0]]
        #                 #         else:

        #                 #             pre_neurons = pre[indexes]
        #                 #             post_neurons = post[indexes]

        #                 #         nest.Connect(pre_neurons,
        #                 #             post_neurons,
        #                 #             {'rule': 'one_to_one', "weight":conn_weights[conn]},
        #                 #             {'vt_num': float(post_cell)},
        #                 #             )
            ...
            for conn in conn_weights.keys():
                ...
                if "pf_pc" in conn:
                    if plasticity:
                        ...
                        vt[receiver[conn]] = nest.Create("volume_transmitter_alberto",len(np.unique(post)))
                        ...
                        nest.CopyModel('stdp_synapse_sinexp', name_plast)
                        nest.SetDefaults(name_plast,{"A_minus": LTD,   # double - Amplitude of weight change for depression
                                                    "A_plus": LTP,   # double - Amplitude of weight change for facilitation
                                                    "Wmin": 0.0,    # double - Minimum synaptic weight
                                                    "Wmax": 2.0,     # double - Maximum synaptic weight
                ...
                if conn == "io_pc" and plasticity:                                     
                    ...
                    nest.Connect(np.asarray(pre, int), np.asarray(post_n, int), {"rule": "one_to_one"},{"model": "static_synapse", "weight": 1.0, "delay": 1.0})
    ...
    def create_ctxinput(self, nest_, pos_file=None, in_spikes='poisson', n_spike_generators='n_glomeruli',
        ...
        if in_spikes == 'spike_generator':
            ...
            CS_matrix_start = np.random.uniform(CS["start"],CS["stop"],[len(cereb.Cereb_pops['glomerulus']), spike_nums_CS])
            ...
            US = ()
            for ii in range(int(len(IO_id))):
                US_new = nest.Create('spike_generator')
                nest.SetStatus(US_new, {'spike_times': US_array})
                US = US + US_new

            # Connection to first half of IO, corresponding to first microzone
            syn_param = {"model": "static_synapse", "weight":55.0, "delay": 1.,"receptor_type":1}
            nest.Connect(US,IO_id[:int(len(IO_id))],{'rule':'one_to_one'},syn_param)
...
# set number of kernels
    ...
    # cereb = Cereb_class(nest, hdf5_file_name, cortex_type, n_spike_generators='n_glomeruli',
    #              mode='external_dopa', experiment='EBCC', dopa_depl=0, LTD=-0.000005, LTP=1e-5)
    ...
    ct = cereb.create_ctxinput(nest, pos_file=hdf5_file_name, in_spikes='EBCC2', 
    ...
    #     cereb.create_ctxinput(nest, pos_file=hdf5_file_name, in_spikes='poisson', n_spike_generators='n_glomeruli',
    #                     experiment='EBCC', CS ={"start":500.+(trial*len_trial), "stop":760.+(trial*len_trial), "freq":36.}, US ={"start":750.+(trial*len_trial), "stop":760.+(trial*len_trial), "freq":500.})
    #     nest.Simulate(len_trial)

        
    # rasters = utils.get_spike_values(nest, sd_list, Cereb_recorded_names)
    # with open(f'./cereb_test/rasters', 'wb') as pickle_file:
    #     pickle.dump(rasters, pickle_file)


    # with open(f'./cereb_test/model_dict', 'wb') as pickle_file:
    ...
    # n_mf = 24
    # glom_id, _ = cereb.get_glom_indexes(cereb.Cereb_pops['glomerulus'], "EBCC")
    ...
    for trial in range(tot_trials):
        ...
        CS_matrix_start_pre = np.round((np.linspace(100.0, 228.0, 11)))
        CS_matrix_start_post = np.round((np.linspace(240.0, 368.0, 11)))
        # CS_matrix_first_pre = CS_matrix_start_pre + set_time +trial*len_trial
        # CS_matrix_first_post = CS_matrix_start_post + set_time +(trial*len_trial)
        ...
        # CS_spk = np.around(np.linspace(CS["start"]+ set_time +(trial*len_trial),CS["stop"]+ set_time +(trial*len_trial),int(CS["freq"]*1000/(CS["stop"]-CS["start"]))), decimals=1)
        CS_stim = nest.Create("spike_generator", n_mf)
        for sg in range(len(CS_stim)):	
            nest.SetStatus(CS_stim[sg : sg + 1], params={"spike_times": CS_matrix[sg].tolist()})
            nest.Connect(CS_stim[sg : sg + 1], glom_mf[sg].tolist())
        ...
        '''
        # US_spk = np.around(np.arange(US["start"]+ set_time +(trial*len_trial),US["stop"]+2+ set_time +(trial*len_trial),2), decimals=1)
    ...
    rasters = utils.get_spike_values(nest, sd_list, Cereb_recorded_names)
    with open(f'./cereb_test/rasters_LTP_'+str(LTP)+"_LTD_"+str(LTD), 'wb') as pickle_file:
        pickle.dump(rasters, pickle_file)


    with open(f'./cereb_test/model_dict_LTP_'+str(LTP)+"_LTD_"+str(LTD), 'wb') as pickle_file:
        pickle.dump(model_dict, pickle_file)